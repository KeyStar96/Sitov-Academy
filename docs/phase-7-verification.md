...
- Passwort-Mindestlänge angeglichen (Code fertig, Live-Rollout offen): `deploy/vps/patch-auth-hibp.py` heißt jetzt `deploy/vps/patch-auth-password-policy.py` und setzt zusätzlich `GOTRUE_PASSWORD_MIN_LENGTH=8` (GoTrue-Standard 6, App 8). Backups künftig unter `/root/backups/sitov-auth-password-policy/`. Bestehende Passwörter bleiben gültig; geprüft wird nur beim Setzen. Tests: `deploy/vps/tests/test_auth_password_policy.py`. Live (24.09.2026): App-Release `87c65445b86e` aktiv, danach Auth; Backup `/root/backups/sitov-auth-password-policy/20260924T090810726896Z-docker-compose.yml` (SHA256 `99c6ee0b…`, = Stand nach HIBP), neue Compose SHA256 `9485dfd6…`; Auth nach ~8 s `healthy`, 0 Fehlerzeilen, Limits aller 20 Container identisch, erneuter Dry-Run `changed: false`; 7-Zeichen-Passwort über Kong → `422 weak_password`, Grund `length`. Achtung: GoTrue nimmt `name@invalid` als gültige Adresse an – Prüfungen mit gültigem Passwort legen ein Konto an.
...
## Nachtrag: `dynamicParams = false` entfernt

`dynamicParams = false` im `[lang]`-Layout ließ nach jedem `revalidatePath('/', 'layout')` (An-, Abmeldung, Passwortänderung in `app/actions/auth.ts`) **alle** statischen Seiten dauerhaft mit 404 antworten (`/de`, `/en`, `/de/registration`, `/de/agb` …; Server-Log `NoFallbackError`). Am Produktions-Build nachgestellt: vorher 200, nach einer Revalidierung 404 mit `x-nextjs-cache: HIT`. Ohne `dynamicParams = false` bauen sich die Seiten neu auf (200, `MISS` → `HIT`). Die Crawling-Falle bleibt geschlossen: Das Layout ruft für unbekannte Sprachen weiter `notFound()` auf (`/authx`, `/apifoo/x`, `/de/xx` → 404).
