...
Im Lehrer-Dashboard unter Schüler → Trainer-Freigaben ein Niveau öffnen und Vokabeltrainer, Grammatikübungen, Aussprache-Training oder Lernvideos einzeln aktivieren/deaktivieren. Die Anzeige zählt aktive Trainer (z. B. 2/4). Gesperrte Studentenkarten haben Schloss, Text und gestrichelten Rahmen und sind nicht verlinkt. Direktrouten und Datenzugriffe sind geschützt. Die rückwärtskompatiblen Migrationen wurden im gebundenen Supabase-Projekt eingespielt; bestehende Rechte und Datensätze sind unverändert. [Umfang und Prüfnachweise](docs/trainer-access-2026-09-10.md).
...
**Kernänderungen:**
- **Einstufung deckt wie die Lernbox auf:** `LessonAssessmentClient` zeigt zuerst die Übersetzung (aufgelöste Muttersprache/Oberfläche), nicht das deutsche Wort. Die Buttons „In den Lernkasten" / „Kenne ich bereits" erscheinen erst nach „Lösung aufdecken". Bekannt bleibt Phase 6, unbekannt Phase 1. Kartenlayout analog zur Lernsession (Bildslot, Plural, Audio, feste Höhe). `getLessonCards` liefert dafür `image_url` und `audio_url`.

## 1a. Änderungsprotokoll — 2026-09-09: Cinematic Preloader

**Kernänderungen:**
- **Ladebildschirm:** `components/effects/Preloader.tsx` ist ein vollflächiger Vorhang mit typografischem 000–100-Zähler, Marken-Reveal (Sitov / Language / Academy), orangener Fortschrittslinie, Radial-Glow und Noise. Exit als zweigeteilter Wipe. Mobil: `100dvh`, Safe-Area, kein Horizontal-Overflow. `prefers-reduced-motion` überspringt die Choreografie. Light Mode nutzt Sand `#FCF4E6` und Charcoal-Typo (`#2D3436`), Dark Mode bleibt `#050505` / `#E2D7CE` – gesteuert über die bestehende `html.dark`-Klasse. Markenzeilen haben `leading-none` plus Clip-Padding, damit Unterlängen (z. B. das g in Language) nicht abgeschnitten werden.

## 1a. Änderungsprotokoll — 2026-09-09: Vokabeltrainer Einstufung, Gewichtung & Layout-Shift

**Kernänderungen:**
- **Ersteinstufung wie Lernbox:** `/vocabulary/assess` zeigt zuerst die Übersetzung, nach „Lösung aufdecken" das deutsche Wort (Artikel, Plural, Audio). Buttons: „Kenne ich bereits" (Phase 6) und „In den Lernkasten" (Phase 1, sofort fällig).
...
- **„Vokabeln einstufen"-Modus (Pre-Assessment):** Neue Route `/dashboard/level/[level]/vocabulary/assess?lesson=…` (Server-Page + `LessonAssessmentClient`, jeweils mit `loading.tsx`/`error.tsx`). Ablauf seit 2026-09-09 wie in der Lernbox: zuerst die Übersetzung, nach „Lösung aufdecken" das deutsche Wort, dann Entscheidung „Kenne ich bereits" (Phase 6) oder „In den Lernkasten" (Phase 1, sofort fällig). Jede Entscheidung wird einzeln und sofort gespeichert. Der Lektions-Button in der Übersicht heißt „Vokabeln einstufen" bzw. „Weitere Vokabeln einstufen".
