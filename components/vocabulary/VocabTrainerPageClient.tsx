...
export default function VocabTrainerPageClient({ initialCards, lessonStats, translations = {}, lang, level, initialDeferredCount = 0, initialPreviousCardId = null }: Props) {
  ...
  return <div className="mx-auto w-full max-w-5xl space-y-8 text-[var(--foreground)]">
    <header className="flex flex-wrap items-end justify-between gap-5">
      <div className="max-w-xl"><Link href={`/${lang}/dashboard/level/${encodeURIComponent(level)}`} className="inline-flex min-h-11 items-center text-sm font-semibold text-[var(--muted)]">← {t('back_to_level')}</Link>
    ...
    <section aria-label={t('your_sets')}>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2"><h2 className="text-xl font-semibold">{t('your_sets')}</h2>
        <button type="button" onClick={() => setSelection(Array.from(dueByLesson.keys()))} className="min-h-11 rounded-xl px-3 text-sm font-semibold text-[var(--accent)]">{t('lernkasten_select_all')}</button>
      </div>
        ...
        {lessonStats.map(lesson => <article key={lesson.lesson} className="min-w-0 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 sm:p-5">
          <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs text-[var(--muted)]">{t('lesson_label', { lesson: stripLessonPrefix(lesson.lesson) })}</p><h3 className="mt-1 break-words text-lg font-semibold">{lesson.lesson}</h3></div>
            <button type="button" onClick={() => toggle(lesson)} aria-pressed={selection.includes(lesson.lesson)} aria-label={t(selection.includes(lesson.lesson) ? 'lernkasten_remove_aria' : 'lernkasten_add_aria', { lesson: lesson.lesson })}
              className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-[var(--border)] text-[var(--accent)]">{selection.includes(lesson.lesson) ? <Check size={20} aria-hidden="true" /> : <Plus size={20} aria-hidden="true" />}</button>
          </div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button type="button" onClick={() => setOpenLesson(lesson.lesson)} className="inline-flex min-h-11 items-center gap-2 rounded-xl px-2 text-sm font-semibold"><BookOpen size={16} aria-hidden="true" />{t('show_cards')}</button>
            {lesson.untouched > 0 && <Link className="inline-flex min-h-11 items-center rounded-xl px-2 text-sm font-semibold text-[var(--accent)]" href={`${overview}/assess?lesson=${encodeURIComponent(lesson.lesson)}`}>{t('assess_set')}</Link>}
          </div>
        </article>)}
      </div>
