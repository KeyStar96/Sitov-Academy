  ...
  return <article className={`min-w-0 overflow-hidden rounded-3xl border bg-[var(--surface)] ${conversation.hasUnseen ? 'border-[var(--accent)]' : 'border-[var(--border)]'}`}>
    ...
    {open && <div id={`conversation-${conversation.id}`} className="border-t border-[var(--border)]">
      ...
      <div className="space-y-4 p-4 sm:p-6"><label htmlFor={`message-${conversation.id}`} className="block text-base font-semibold">{t('reply_label')}</label><textarea id={`message-${conversation.id}`} value={text} onChange={(event) => setText(event.target.value)} disabled={sending} maxLength={5000} rows={3} placeholder={t('reply_placeholder')} className="w-full resize-y rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 text-base focus:outline-2 focus:outline-[var(--accent)] disabled:opacity-50" />
        ...
        {notice && <p role="status" className={`text-base ${notice === 'error' ? 'text-[var(--danger)]' : 'text-[var(--success)]'}`}>{t(notice === 'error' ? 'message_failed' : 'message_sent')}</p>}
        <div className="flex flex-wrap justify-between gap-3">{recorder.isRecording ? <button type="button" className={button} onClick={recorder.stop}><Square size={20}/>{t('stop_recording')}</button> : !recorder.hasRecording && <button type="button" className={button} disabled={sending || recorder.status === 'requesting'} onClick={() => void recorder.start()}>{recorder.status === 'requesting' ? <Loader2 size={20} className="animate-spin"/> : <Mic size={20}/>} {t('voice_message')}</button>}<button type="button" disabled={sending || recorder.isRecording || (!text.trim() && !recorder.audioBlob)} onClick={() => void send()} className="ml-auto inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[var(--accent)] px-5 py-3 text-base font-semibold text-[var(--accent-foreground)] disabled:opacity-50">{sending ? <Loader2 size={20} className="animate-spin"/> : <Send size={18}/>} {t(sending ? 'sending_message' : 'send_message')}</button></div>
      </div>
    </div>}
  </article>
}
