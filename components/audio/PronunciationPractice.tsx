...
export default function PronunciationPractice({ prompts, level, translations }: { prompts: readonly PronunciationPrompt[]; level: string; translations?: PronunciationTranslations }) {
  ...
  if (!selected) return <section className="rounded-[2rem] border border-dashed border-[var(--border)] p-10 text-center"><BookOpen className="mx-auto mb-4 text-[var(--accent-text)]" size={32} /><h2 className="text-xl font-semibold">{t('prompts_empty')}</h2><p className="mt-3 text-[var(--muted)]">{t('prompts_empty_hint')}</p></section>
  const wordCount = selected.sentenceDe.split(/\s+/).length
  return <div className="pronunciation-practice space-y-6">
    <ol className="grid gap-3 sm:grid-cols-3">
      {([{ icon: BookOpen, key: 'step_read' }, { icon: Mic, key: 'step_record' }, { icon: MessageCircle, key: 'step_feedback' }] as const).map(({ icon: Icon, key }, index) => <li key={key} className="flex items-center gap-3 rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-4 py-4"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--surface-muted)] text-[var(--accent-text)]"><Icon size={20} /></span><span className="text-base font-semibold"><span className="mr-2 text-[var(--muted)]">0{index + 1}</span>{t(key)}</span></li>)}
    </ol>
    <div className="grid items-start gap-6 lg:grid-cols-[260px_minmax(0,1fr)]">
      ...
      <div className="min-w-0 space-y-5">
        ...
        <p className="flex items-start gap-3 px-2 text-base leading-relaxed text-[var(--muted)]"><Headphones size={20} className="mt-0.5 shrink-0 text-[var(--accent-text)]" />{t('reading_tip')}</p>
