...
export default async function AuthShell({lang,title,description,children}:{lang:string;title:string;description?:ReactNode;children:ReactNode}) {
  ...
  return <div className="min-h-dvh bg-[var(--canvas)] px-4 py-5 text-[var(--foreground)] sm:px-7 sm:py-7">
    ...
    <main className="mx-auto grid w-full max-w-6xl min-w-0 items-center gap-7 py-10 sm:py-16 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:gap-16">
      <section className="relative min-w-0 lg:py-12">
        ...
        <div className="mt-9 hidden max-w-sm items-center gap-4 lg:flex" aria-hidden="true"><span className="flex h-20 w-20 items-center justify-center rounded-3xl border border-[var(--border)] bg-[var(--surface)] text-[var(--violet)] shadow-sm"><BookOpen size={30}/></span><span className="h-px flex-1 bg-[var(--border)]"/><span className="flex h-20 w-20 items-center justify-center rounded-3xl bg-[var(--accent)] text-[var(--accent-foreground)] shadow-sm"><MessageCircle size={30}/></span></div>
      </section>
      <section className="min-w-0 rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-[0_16px_70px_-30px_rgba(60,35,100,0.22)] sm:p-8">
