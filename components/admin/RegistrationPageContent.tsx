...
export default async function RegistrationPageContent({lang,month,mode}:{lang:string;month?:string;mode:'registrations'|'invoices'}) {
  const [result,dict]=await Promise.all([getRegistrationOverview(month&&/^\d{4}-\d{2}$/.test(month)?`${month}-01`:undefined),getDictionary(lang)])
  ...
  return <RegistrationDesk key={`${mode}:${result.data.targetMonth}`} initial={result.data} lang={lang} mode={mode} courseTitles={Object.fromEntries(Object.entries(courses).map(([key,value])=>[key,value.title??key]))}/>
}
