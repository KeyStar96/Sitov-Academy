      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-5 sm:p-6" data-lenis-prevent>
        <section className="mb-6" aria-label={t('detail_contact')}>
          ...
          <dl className="mt-3 grid gap-3 text-base leading-relaxed sm:grid-cols-2">
            {[[t('email'), student.person?.email], [t('phone'), student.person?.phone || t('not_specified')], [t('street'), address || t('not_specified')]].map(([label, value]) => <div key={label}><dt className="font-semibold text-[var(--muted)]">{label}</dt><dd className="break-words">{value}</dd></div>)}
          </dl>
        </section>
