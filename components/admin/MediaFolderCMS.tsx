...
import type { MediaCourse, MediaFolder } from '@/lib/media'
import MediaFolders from '@/components/dashboard/MediaFolders'
import MediaUpload from './MediaUpload'

export default function MediaFolderCMS({ initial, courses, lang }: { initial: MediaFolder[]; courses: MediaCourse[]; lang: string }) {
  const t = mediaCopy(lang)
  const [folders, setFolders] = useState(initial)
  const [selected, setSelected] = useState(initial[0]?.folder_id ?? '')
  const empty = { title: '', level: 'A1.1', course_id: '', sort_order: 0, folder_id: undefined as string | undefined }
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      const result = await saveMediaFolder({ ...form, course_id: form.course_id || null })
  ...
  return <div className="grid items-start gap-6 xl:grid-cols-[20rem_minmax(0,1fr)]"><aside className="min-w-0 space-y-6">
    ...
    {folder && <button onClick={() => setForm({ folder_id: folder.folder_id, title: folder.title, level: folder.level, sort_order: folder.sort_order, course_id: folder.course_id ?? '' })} className={`${field} text-left`}>{t.edit}</button>}
    <form onSubmit={save} className="space-y-4 rounded-2xl border border-[var(--border)] p-5"><h2 className="text-lg font-semibold">{form.folder_id ? t.edit : t.newFolder}</h2>
      ...
      <label className="block space-y-2"><span>{t.level}</span><select disabled={!!form.folder_id} value={form.level} onChange={event => setForm({ ...form, level: event.target.value, course_id: '' })} className={field}>{ACCESS_LEVELS.map(level => <option key={level}>{level}</option>)}</select></label>
      <label className="block space-y-2"><span>{t.course}</span><select value={form.course_id} onChange={event => setForm({ ...form, course_id: event.target.value })} className={field}><option value="">{t.allCourses}</option>{courses.filter(course => course.level === form.level).map(course => <option key={course.id} value={course.id}>{course.title}</option>)}</select></label>
