...
import { Check, FileText, Loader2, Search, UserRound, ExternalLink, CheckCircle2 } from 'lucide-react'
import { confirmRegistration, saveManualInvoiceStatus } from '@/app/actions/admin-registrations'
...
export default function RegistrationDesk({ initial, lang, mode, courseTitles = {} }: { initial: RegistrationOverview; lang:string; mode:'registrations'|'invoices';courseTitles?:Record<string,string> }) {
  ...
  return <div className="mx-auto w-full min-w-0 max-w-6xl space-y-7 [overflow-wrap:anywhere]">
    ...
    <div className="flex flex-wrap gap-2" aria-label={mode==='registrations'?t.registrations_title:t.invoices_title}>{(mode==='registrations'?['pending','confirmed','all']:['outstanding','created','all']).map(value=><button key={value} type="button" onClick={()=>setFilter(value)} aria-pressed={filter===value} className={`${control} ${filter===value?'border-[var(--violet)] bg-[var(--violet)]/10 font-bold':''}`}>{t[value as 'pending'|'confirmed'|'all'|'outstanding'|'created']}</button>)}</div>
...
function RegistrationCard({row,invoice,lang,mode,busy,disabled,onAccept,onInvoice,courseTitles}:{row:StaffRegistration;invoice?:StaffInvoice;lang:string;mode:'registrations'|'invoices';busy:boolean;disabled:boolean;onAccept:()=>void;onInvoice:(created:boolean,reference:string)=>void;courseTitles:Record<string,string>}) {
  ...
  return <article className="min-w-0 overflow-hidden rounded-3xl border border-[var(--border)] bg-[var(--surface)] shadow-sm">
    <div className="flex flex-col justify-between gap-4 border-b border-[var(--border)] px-5 py-5 sm:flex-row sm:items-center sm:px-7"><div className="min-w-0"><p className="mb-1 text-xs font-bold uppercase tracking-widest text-[var(--muted)]">{row.source==='registration'?t.registration:t.monthly}</p><h2 className="text-xl font-bold text-[var(--foreground)]">{row.contact.name}</h2><a href={`mailto:${row.contact.email}`} className="inline-flex min-h-12 items-center text-[var(--muted)] underline underline-offset-4">{row.contact.email}</a></div><span className={`inline-flex w-fit rounded-full px-4 py-2 text-sm font-semibold ${status==='created'||status==='confirmed'?'bg-emerald-50 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200':'bg-amber-50 text-amber-900 dark:bg-amber-950 dark:text-amber-200'}`}>{t[status]}</span></div>
    <div className="grid min-w-0 gap-6 p-5 sm:p-7 lg:grid-cols-2">
      ...
      <section className="min-w-0"><h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-[var(--muted)]">{t.courses}</h3><ul className="space-y-2">{row.courses.map(course=><li key={course.id} className="flex justify-between gap-3 rounded-xl bg-[var(--canvas)] p-3 text-[var(--foreground)]"><span>{courseTitles[course.translationKey]??course.title}</span>{course.price!==null&&<span className="shrink-0 font-semibold">{currency(course.price)}</span>}</li>)}</ul>{row.totalPrice!==null&&<p className="mt-4 flex flex-wrap justify-between gap-2 text-sm text-[var(--muted)]"><span>{t.total}</span><strong className="text-base text-[var(--foreground)]">{currency(row.totalPrice)}</strong></p>}</section>
    </div>
    ...
    <footer className="flex flex-col gap-3 border-t border-[var(--border)] bg-[var(--canvas)]/50 p-5 sm:p-7">
      {mode==='registrations'&&row.status==='pending'&&<button type="button" onClick={onAccept} disabled={disabled} aria-busy={busy} className={`${primary} self-start`}>{busy?<Loader2 className="animate-spin" size={18}/>:<Check size={18} aria-hidden="true"/>}{busy?t.saving:t.accept}</button>}
