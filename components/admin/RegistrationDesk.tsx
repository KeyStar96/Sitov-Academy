...
const primary = 'inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[var(--accent-strong)] px-5 py-3 font-semibold text-[var(--accent-foreground)] transition-opacity hover:bg-[var(--accent-strong-hover)] disabled:opacity-50'
...
export default function RegistrationDesk({ initial, lang, mode }: { initial: RegistrationOverview; lang:string; mode:'registrations'|'invoices' }) {
  ...
  return <div className="mx-auto w-full min-w-0 max-w-6xl space-y-7 [overflow-wrap:anywhere]">
    ...
    <div className="flex flex-wrap gap-2" aria-label={mode==='registrations'?t.registrations_title:t.invoices_title}>{(mode==='registrations'?['pending','confirmed','cancelled','all']:['outstanding','created','all']).map(value=><button key={value} type="button" onClick={()=>setFilter(value)} aria-pressed={filter===value} className={cn(control,filter===value&&'border-[var(--violet)] bg-[color-mix(in_srgb,var(--violet)_10%,var(--surface))] font-bold')}>{t[value as 'pending'|'confirmed'|'cancelled'|'all'|'outstanding'|'created']}</button>)}</div>
