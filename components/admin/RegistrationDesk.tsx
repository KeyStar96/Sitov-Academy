    ...
    {declineRow&&<AdminDialog title={declineRow.isTrial?t.decline_trial_title:t.decline_title} subtitle={declineRow.contact.name} dismissible={!pending} onClose={()=>{if(!pending)setDeclineRow(null)}}>
      <div className="min-h-0 space-y-5 overflow-y-auto overscroll-contain p-5 sm:p-6">
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--canvas)] p-4">
          ...
          <ul className="mt-3 space-y-2">{declineRow.courses.map(course=><li key={course.id}>{courseTitles[course.translationKey]??course.title}</li>)}</ul>
...
function RegistrationCard({row,invoice,lang,mode,busy,disabled,onAccept,onDecline,onInvoice,courseTitles}:{row:StaffRegistration;invoice?:StaffInvoice;lang:string;mode:'registrations'|'invoices';busy:boolean;disabled:boolean;onAccept:()=>void;onDecline:()=>void;onInvoice:(created:boolean,reference:string)=>void;courseTitles:Record<string,string>}) {
  const t=registrationLabels(lang)
