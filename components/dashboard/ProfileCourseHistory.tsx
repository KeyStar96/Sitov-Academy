...
export default async function ProfileCourseHistory({history,lang}:{history:VerifiedCourseHistory|null;lang:string}) {
  ...
  return <section className="rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-5 sm:p-7"><div className="flex items-start gap-3"><span className="rounded-2xl bg-[color-mix(in_srgb,var(--violet)_10%,transparent)] p-3 text-[var(--violet)]"><BookOpen size={23} aria-hidden="true"/></span><div><h2 className="text-xl font-bold text-[var(--foreground)]">{t.history_title}</h2><p className="mt-2 text-sm leading-relaxed text-[var(--muted)]">{t.history_intro}</p></div></div>
