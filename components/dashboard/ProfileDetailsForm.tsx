    ...
    <section aria-labelledby="personal-title" className="min-w-0 rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-sm sm:p-7">
      <div className="mb-6 flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 break-words text-base leading-relaxed text-[var(--muted)]">{t('personal_intro')}</p>
        </div>
      </div>
      {birthDate && <dl className="mb-5 flex flex-wrap justify-between gap-2 rounded-xl bg-[var(--canvas)] px-4 py-3 text-base"><dt className="text-[var(--muted)]">{registrationLabels(lang).birthday}</dt><dd className="font-semibold text-[var(--foreground)]">{birthDate}</dd></dl>}
      <form onSubmit={submit} noValidate aria-busy={saving}>
        ...
        {pending && <p className="mt-4 break-words rounded-xl bg-amber-50 p-3 text-base text-amber-900 [overflow-wrap:anywhere] dark:bg-amber-950 dark:text-amber-200">{t('pending_email', { email: pending })}</p>}
        ...
        <div className="mt-3 min-h-12 min-w-0 break-words text-base leading-relaxed" role={failed ? 'alert' : 'status'} aria-live={failed ? 'assertive' : 'polite'} aria-atomic="true">
