...
export default function MediaAssetViewer({ asset, lang }: { asset: MediaAsset; lang: string }) {
  ...
  return <article className="min-w-0 space-y-3 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4">
    ...
    <div className="flex flex-wrap gap-2">{(asset.kind === 'videos' || asset.mime === 'application/pdf') && <button disabled={busy} onClick={() => url ? close() : void open()} className="min-h-12 rounded-lg border border-[var(--border)] px-4">{url ? t.close : t.open}</button>}
      <button disabled={busy} onClick={() => void open(true)} className="min-h-12 rounded-lg border border-[var(--border)] px-4">{t.download}</button></div>
    {busy && <p role="status">{t.loading}</p>}{error && <p role="alert">{t.accessFailed}</p>}
    {url && (asset.kind === 'videos' ? <video ref={video} controls playsInline preload="metadata" src={url} aria-label={asset.title} className="aspect-video w-full rounded-lg bg-black" onLoadedMetadata={() => { if (video.current) { video.current.currentTime = position.current; if (resume.current) void video.current.play().catch(() => {}) } }} onError={() => { close(); setError(true) }} />
