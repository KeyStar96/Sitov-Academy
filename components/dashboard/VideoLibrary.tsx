  ...
  return <div className="mx-auto max-w-5xl space-y-8 text-[var(--foreground)]">
    {failed ? <p role="alert" className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6">{t('error_description')}</p> : links.length ? <section className="space-y-3">
      {links.map((video, index) => <a key={video.id} href={video.url} target="_blank" rel="noopener noreferrer" aria-label={t('open_external_aria', { title: video.title })}
        ...
        <div className="min-w-0 flex-1"><p className="text-xs font-medium text-[var(--muted)]">{video.lesson} · YouTube</p><h2 className="mt-1 break-words text-lg font-semibold">{video.title}</h2>{video.description && <p className="mt-2 break-words text-sm leading-relaxed text-[var(--muted)]">{video.description}</p>}<span className="mt-3 inline-flex min-h-12 items-center gap-2 text-sm font-semibold text-[var(--violet)]">{t('watch_youtube')}<ArrowUpRight size={16} aria-hidden="true" /></span></div>
      </a>)}
      ...
      <p className="mt-7 text-xs font-semibold tracking-widest text-[var(--muted)] uppercase">{t('in_preparation')}</p>
      ...
      <Link href={`${levelHref}/vocabulary`} className="mt-7 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-[var(--border)] px-5 text-sm font-semibold"><BookOpen size={18} aria-hidden="true" />{t('continue_vocabulary')}</Link>
    </section>}
    <p className="max-w-2xl text-sm leading-relaxed text-[var(--muted)]">{t('external_privacy')}</p>
  </div>
}
