...
export default function MediaFolders({ folders, lang, onVisibilityChanged, onEditLink }: {
  folders: MediaFolder[]
  lang: string
  onVisibilityChanged?: () => Promise<void>
  onEditLink?: (link: MediaLink, folderId: string) => void
}) {
  ...
  return <div className="space-y-6">{!folders.length && <p>{t.noFolders}</p>}{folders.map(folder => <section key={folder.folder_id} className="space-y-5 rounded-2xl border border-[var(--border)] bg-[var(--surface-muted)] p-4 sm:p-6">
    ...
    {onVisibilityChanged && onEditLink && <section className="space-y-3"><h3 className="text-lg font-medium">{t.links}</h3><div className="grid gap-3 lg:grid-cols-2">{folder.links.map(link => <MediaLinkCard key={link.id} link={link} lang={lang} onEdit={() => onEditLink(link, folder.folder_id)} onChanged={onVisibilityChanged} />)}</div>{!folder.links.length && <p className="text-sm text-[var(--muted)]">{t.empty}</p>}</section>}
  </section>)}</div>
}
