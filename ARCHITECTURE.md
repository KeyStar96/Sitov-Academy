...
Die Anwendung verwendet direkte Tabellenzugriffe auf das standardisierte lokale Supabase-Schema. Die acht Kompatibilitäts-Views, Stripe-Felder/-Routen und abgehängten alten Triggerfunktionen sind entfernt. `profiles` enthält Auth-ID, Rolle und ISO-Sprachcodes; `people.auth_user_id` ist eine eindeutige Fremdschlüsselbeziehung. Natürlich eindeutige Sprach-, Niveau- und Trainercodes besitzen Lookup-Tabellen, eigenständige Entitäten UUIDs.

Der vollständige Kurskatalog enthält neun Datensätze: Deutsch Level 1–3, Sprechtraining Montag–Mittwoch, Deutsch A1.1/B1 Online und Privatunterricht Online. Zwei ausgemusterte Kurse und Privatunterricht Präsenz werden physisch entfernt. Kurse bleiben im CMS dynamisch verwaltbar. `unit_price`, `unit_minutes` und bei privaten Buchungen `requested_units` bilden eine eindeutige Preisgrundlage; Beträge werden in PostgreSQL berechnet und in `booking_items` gespeichert.

Alle 512 Vokabelkarten, 604 Grammatikübungen, 149 Aussprachetexte und drei Lernressourcen bleiben erhalten. Jeder Inhalt hat eine feste UUID-Einheit. Die beiden bestehenden DW-Links bleiben erhalten; eine fehlende Quelle ist ein unveröffentlichter Entwurf. Das Videoschema hat nur `source_url`, keine konkurrierenden URL-/Providerfelder. Onboarding und Lektionsreset verwenden UUIDs. Vokabelrichtungen bleiben getrennt, Einzelfreigaben und Audiodialoge RLS-geschützt. Ein schwarzes Brett pro Schüler ersetzt Rabatt-/Zusatzflags.

Der Betrieb bleibt auf dem eigenen VPS mit lokaler Supabase, Postfix, TTS, privatem Storage und IP-TLS. DNS bleibt unverändert. Aufnahmen und ihr Reset verwenden ausschließlich `pronunciation_audio`; `audio_cache` dient weiterhin der lokalen Sprachsynthese. `deploy-canonical.py` baut vorab gegen einen isolierten Restore und schaltet Schema und Anwendung nach einer erneuten Sicherung zusammen um; ein inkompatibles altes Release wird nicht allein zurückgeschaltet.

[Tabellenvertrag, Sicherung, Mengenregeln und Prüfnachweise](docs/canonical-schema-2026-09-13.md).
