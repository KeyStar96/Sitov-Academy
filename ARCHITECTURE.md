...
**Die normalisierte Migration ist live angewendet; der eigene VPS ist über [https://217.154.228.254](https://217.154.228.254) erreichbar.** Das vertrauenswürdige Let's-Encrypt-IP-Zertifikat und `certbot renew --dry-run` sind geprüft. Das Zertifikat ist etwa sieben Tage gültig; die Erneuerung wird alle zwölf Stunden geprüft. DNS-Einträge bleiben unverändert. Cloud-Supabase, frühere Vercel-Bereitstellungen und externe Mail-/Sprachdienste sind kein Bestandteil dieses Betriebswegs. Die zuletzt erlaubte lesende Cloud-Orientierung wurde nicht genutzt; es gab für diesen Umbau keine Cloud-Zugriffe.

Next.js läuft unter systemd auf `127.0.0.1:3000`; der Reverse Proxy übernimmt HTTPS. Supabase Auth, PostgREST, PostgreSQL und Storage laufen im lokalen Docker-Stack. Unveränderliche Anwendungs-Releases liegen in `/var/www/sitov-releases/<12hash>`, `/var/www/sitov-current` zeigt auf das aktive Release. [deploy-release.sh](deploy/vps/deploy-release.sh) prüft den Dienst nach dem Umschalten und setzt den Symlink bei fehlgeschlagener Gesundheitsprüfung zurück. Es wird kein PM2 verwendet; ein Anwendungs-Rollback ersetzt keinen Datenbank-Restore.

Personenstammdaten liegen in `people`, Rollen und Sprachpräferenzen in `profiles`; `profile_details` ist eine RLS-beachtende Lesesicht. `bookings`, `booking_items` und `invoice_cases` ersetzen die mehrfach geführten Buchungs-/Rechnungstabellen. Kurszeiten und Übersetzungen sind eigene Relationen. Der live geprüfte Katalog enthält zehn aktive und zwei archivierte Kurse, 512 Vokabelkarten, 604 Grammatikübungen sowie 149 Aussprachetexte, davon 60 aktiv.
...
Aufnahmen liegen im privaten Bucket `pronunciation_audio`; signierte URLs prüfen Gespräch, Niveau und Einheit. Der alte Aufnahme-Bucket ist gesperrt. Server verwenden die interne Supabase-Adresse; der gemeinsame Storage-Adapter bildet nur deren exakt konfigurierten Storage-Präfix auf die öffentliche HTTPS-API ab, erhält kodierte Pfade und Signaturparameter und lehnt fremde Ursprünge ab. Die private Aufnahme-/Antwortkette mit Waveform und die lokale Referenzwiedergabe wurden im Browser geprüft.

E-Mails liegen in einer lokalen Outbox mit Deduplizierung, Lease und begrenztem Backoff. Postfix nimmt Anwendungsmail auf `127.0.0.1:25` an; Auth verwendet den lokalen Host-SMTP-Zugang auf Port `2525` aus Docker. Eine Auth-Mail wurde an einen lokalen Testempfänger zugestellt und ihr Token bestätigt; ein nativer Worker-Lauf mit `--once` setzte den Testauftrag auf `sent`. Es wurden keine externen Testmails versendet. Der regelmäßige Mailworker ist aktiviert und läuft unter systemd. Piper erzeugt Deutsch, Englisch, Russisch und Ukrainisch lokal; Türkisch nutzt eSpeak-NG. Alte Cloud-HTTP-Jobs sind entfernt.

Der letzte dokumentierte vollständige Prüflauf umfasst 82 Jest-Suites mit 1.079 bestandenen Tests, einen übersprungenen Opt-in-Netzwerktest, 60 isolierte PostgreSQL-/Worker-Tests und fehlerfreies TypeScript. Live bestanden 22 REST-Prüfgruppen. Die Chat-Korrektur ist mit Release `af37ee8b050e` live bestätigt: neue Nachrichten erschienen im offenen Dialog nach 10,9 Sekunden ohne Neuladen. Testkonten und Testaufnahmen sind nach einer zusätzlichen Sicherung entfernt. [Datenmodell, Live-Nachweise und Sicherung](docs/vps-refactor-2026-09-13.md).
...
- **Progress & User Data**:
  - `user_vocabulary_progress`: Phase-6-Leitner (`box_number` 1–6 = aktive Lernphasen, 7 = dauerhaft gelernt), `next_review_date`, `lapses` (Anzahl der Rückstufungen), `last_answered_at`.
    ...
    - Session-Reihenfolge: `getDueCards` zieht alle fälligen Karten (`box_number < 7` und `next_review_date <= now`) und sortiert sie per Weighted-Random (`PHASE_SELECTION_WEIGHTS`: 1.0 / 0.7 / 0.4 / 0.2 / 0.1 / 0.05).
    - Index `idx_user_vocabulary_progress_due` auf `(user_id, next_review_date, box_number)` für die Abfrage der fälligen Karten.
    - **Übernahme-Wege in den Karteikasten:** (1) automatisch beim Sofort-Start einer ganzen Lektion (`initializeLesson`, legacy), (2) manuell je Einzelvokabel über `addCardsToTrainer(cardIds)` (Start immer Phase 1), (3) über den „Vokabeln einstufen"-Durchlauf `submitLessonAssessment(decisions)` – die UI zeigt zuerst die Übersetzung und nach „Lösung aufdecken" das deutsche Wort; bekannt → Phase 6, unbekannt → Phase 1 und sofort fällig. Nach unbekannter Einstufung startet die Lernsession ohne Umweg. Alle drei Wege überspringen Karten mit bereits bestehendem Lernstand (kein Überschreiben von Fortschritt).
...
- **Vokabeltrainer-Kartenwechsel (feste Höhe, seit 2026-09-09):** `VocabCardSession.tsx` hält aktuelle Karte (`i`) und Folgekarte (`i+1`) im selben CSS-Grid-Slot mit identischer fester Höhe. Keine Skalierung der Folgekarte, einheitlicher Bildslot, reservierte Rückseite – der Wechsel erzeugt keinen Layout-Shift. Die Selbsteinschätzung speichert fire-and-forget (`submitVocabularyAnswer`); die aktive Karte fliegt in ~260 ms zur Seite. `finishVocabularySession` läuft nach der letzten Karte.
