...
## Stand dieses Releases — Mengenpreise und manuelle Rechnungen

Kursbuchung, Rechnungsarbeit und pädagogische Freigaben bleiben getrennt. Trainerzugang wird ausdrücklich durch Lehrkräfte vergeben; ein Zahlungsstatus schaltet keine Inhalte frei. Stripe-Endpunkte, Stripe-Profilfelder und die ungenutzte Abhängigkeit sind entfernt. Rechnungen werden weiter am Monatsende für den Folgemonat manuell in papierkram.de erstellt; `invoice_cases` dokumentiert offen/erstellt.

Die Datenbank enthält neun Kurse. Privatunterricht Online kostet 25 € je 45 Minuten, mit frei gewählter ganzer Anzahl von 1–1.000 Einheiten. Die DB speichert gewählte Einheiten, Stückpreis, Dauer und Betrag als Buchungsgrundlage. Vier Einheiten ergeben 100 €. Der Monatsprozess übernimmt diese Menge; Schüler können den Folgemonat bis zur Rechnung ändern oder mit leerer Auswahl pausieren. Preisänderungen im Katalog verändern bestehende Buchungssnapshots nicht. Bereits verwendete Kurse wechseln nicht nachträglich zwischen privatem Mengen- und Kalenderpreismodell.

Kurse bleiben dynamisch anlegbar/bearbeitbar/archivierbar. Die beiden ausgemusterten Altkurse und Privatunterricht Präsenz werden nach Backup physisch entfernt. Es gibt genau ein schwarzes Brett pro Schüler und kein Rabattfeld. Personen werden nur anhand einer verifizierten, eindeutigen Auth-Identität zugeordnet; öffentliche Anmeldungen geben keine bestehenden Personendaten preis.

[Verbindlicher Tabellenvertrag und Prüfnachweise](docs/canonical-schema-2026-09-13.md). Anwendung, Supabase, Postfix und Sprachsynthese laufen ausschließlich über den eigenen VPS; bestehendes IP-TLS und DNS bleiben unverändert.
