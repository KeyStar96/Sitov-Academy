  ...
  test.describe(role, () => {
    ...
    for (const theme of ['light', 'dark']) for (const route of routes[role]) {
      test(`${theme} ${route}`, async ({ page }, info) => {
        ...
        if (route === '/en/dashboard/lessons') await expect(page).toHaveURL('/en/dashboard')
        if (route === '/en/admin/feedback') await expect(page).toHaveURL('/en/admin/submissions')
        if (route === '/en/admin/content/videos') await expect(page).toHaveURL('/en/admin/content/media')
        if (route === `${level}/media` || route.includes('/videos/')) await expect(page).toHaveURL(`${level}/videos`)
