...
export async function getCourseCatalog():Promise<BackendActionResult<CourseEditor[]>> {
 return withBackendSession(async({supabase})=>{
  ...
  return (data??[]).map(row=>courseEditorSchema.parse({
    id:row.id,slug:row.slug,title:row.title,description:row.description,type:row.type,category:row.category,level:row.level,price:row.price,
    unit_duration:row.unit_duration,instructor:row.instructor,start_date:row.start_date??'',end_date:row.end_date??'',trial_lessons:row.trial_lessons,sort_order:row.sort_order,archived:!!row.archived_at,
    schedules:row.course_schedules.map(s=>({weekday:s.weekday,start_time:s.start_time.slice(0,5),end_time:s.end_time.slice(0,5),alternate_start_time:s.alternate_start_time?.slice(0,5)??'',alternate_end_time:s.alternate_end_time?.slice(0,5)??''})),
