  ...
  return withBackendSession(async ({ supabase }) => {
    ...
    for (let offset = 0; offset < folders.length; offset += 100) {
      ...
      const [videos, presentations] = await Promise.all([
        readAllRows((from, to) => supabase.from('learning_videos').select('id,folder_id,title,storage_path,file_size').in('folder_id', ids).not('storage_path', 'is', null).order('created_at').order('id').range(from, to)),
      ...
      result.push(...batch.map(folder => ({ ...folder, assets: [
        ...videos.filter(asset => asset.folder_id === folder.folder_id).map(asset => ({ id: asset.id, title: asset.title!, path: asset.storage_path!, bytes: asset.file_size!, mime: asset.storage_path!.endsWith('.webm') ? 'video/webm' : 'video/mp4', kind: 'videos' as const })),
