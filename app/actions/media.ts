...
export async function getMediaFolders(level?: string) {
  return withBackendSession(async ({ supabase }) => {
    ...
    const folders = await readAllRows((from, to) => {
      let query = supabase.from('lms_media_folder').select('folder_id,level,course_id,title,sort_order').order('sort_order').order('title').order('folder_id')
    ...
    for (let offset = 0; offset < folders.length; offset += 100) {
      ...
      const [videos, presentations] = await Promise.all([
        readAllRows((from, to) => supabase.from('learning_videos').select('id,folder_id,title,storage_path,file_size,unit:learning_units!inner(is_active)').in('folder_id', ids).not('storage_path', 'is', null).order('created_at').order('id').range(from, to)),
        readAllRows((from, to) => supabase.from('lms_presentation_asset').select('asset_id,folder_id,file_name,storage_path,mime_type,file_size').in('folder_id', ids).order('sort_order').order('file_name').order('asset_id').range(from, to)),
      ])
      result.push(...batch.map(folder => ({ ...folder, assets: [
        ...videos.filter(asset => asset.folder_id === folder.folder_id).map(asset => ({ id: asset.id, title: asset.title!, path: asset.storage_path!, bytes: asset.file_size!, mime: asset.storage_path!.endsWith('.webm') ? 'video/webm' : 'video/mp4', kind: 'videos' as const, isActive: asset.unit.is_active })),
        ...presentations.filter(asset => asset.folder_id === folder.folder_id).map(asset => ({ id: asset.asset_id, title: asset.file_name, path: asset.storage_path, bytes: asset.file_size, mime: asset.mime_type, kind: 'presentations' as const })),
      ] })))
    }
    return result
  })
