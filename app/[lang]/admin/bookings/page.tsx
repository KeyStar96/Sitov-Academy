...
export default async function AdminBookingsPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  if (overviewResult.success === false) {
    return (
      <div className="rounded-3xl border border-amber-200 bg-amber-50 p-6 dark:border-amber-800 dark:bg-amber-950">
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white">{t('bookings_title')}</h1>
  ...
  return (
    <div className="space-y-8">
      <div>
        <p className="text-sm font-bold text-orange-800 dark:text-orange-300">{formatProfileMonth(overview.targetMonth, lang)}</p>
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white">{t('bookings_title')}</h1>
