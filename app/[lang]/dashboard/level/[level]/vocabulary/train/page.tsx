...
export default async function VocabularyTrainPage({ params, searchParams }: {
  ...
  return <VocabCardSession cards={lesson ? session.cards.filter(item => item.card.lesson === lesson) : session.cards} initialDeferredCount={session.deferredCount} previousCardId={session.previousCardId}
