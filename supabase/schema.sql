...
CREATE FUNCTION learning_private.reset_student_level(p_student_id uuid, p_level text) RETURNS void
 ...
 PERFORM path_private.reset_progress(p_student_id,u.id) FROM public.learning_units u WHERE u.level=p_level AND u.is_path; DELETE FROM public.user_exercise_progress p USING public.learning_exercises e,public.learning_units u WHERE u.id=e.unit_id AND p.exercise_id=e.id AND p.auth_user_id=p_student_id AND u.level=p_level;
...
BEGIN
 ...
 PERFORM path_private.reset_progress(actor); DELETE FROM public.user_exercise_progress WHERE auth_user_id=actor;
 DELETE FROM learning_reset_private.audio_objects WHERE auth_user_id=actor;
