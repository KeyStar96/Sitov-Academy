...
it('never looks up data for an unverified signup',async()=>{
 expect(await resolveVerifiedPerson({...user,email_confirmed_at:undefined})).toEqual({id:null,unresolved:false})
...
it('accepts only the identity claimed by the authenticated database RPC; email and user ID are not request parameters',async()=>{
 ...
 expect((await resolveVerifiedPerson(user)).id).toBe('00000000-0000-4000-8000-000000000099')
 expect(rpc).toHaveBeenCalledWith('claim_verified_person')
})
