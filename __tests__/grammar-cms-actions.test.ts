...
const input = grammarWriteSchema.parse({
  ...
  translation_prompt: { ru: 'Этот стол.', uk: 'Цей стіл.' },
  content: { target_form: ['bestimmter Artikel'], text_before: '', text_after: ' Tisch.', correct_answer: 'Der', options: ['Der', 'Die', 'Das'], accepted_answers: ['Der', 'Dieser'], smart_hint: { de: 'Erklärung', en: 'Explanation', ru: 'Объяснение' } },
})
...
function setup(role: string) {
  ...
  const row = { id, unit_id: id, topic: input.topic, type: input.type, solution_audio_url: null, content, created_at: null,
    ...
    translations: ['de', 'en', 'ru', 'uk', 'tr'].map(locale => ({ exercise_id: id, locale, prompt: input.translation_prompt?.[locale] ?? null, hint: input.hint?.[locale] ?? null, smart_hint: typeof smart_hint === 'object' && smart_hint !== null ? smart_hint[locale] ?? null : null, explanation: null })),
  }
...
test('teacher updates preserve localized hints and alternatives through the normalized writer', async () => {
  ...
  expect(await saveGrammarExercise(input, id)).toMatchObject({ success: true, data: { hint: input.hint, content: input.content, translation_prompt: input.translation_prompt } })
  expect(client.rpc).toHaveBeenCalledWith('save_learning_content', { p_trainer: 'exercises', p_id: id, p_payload: { unit: { level: input.level, label: input.lesson }, fields: expect.objectContaining({ content: expect.objectContaining({ correct_answer: 'Der', accepted_answers: ['Der', 'Dieser'], target_form: ['bestimmter Artikel'] }) }), translations: expect.arrayContaining([{ locale: 'ru', prompt: 'Этот стол.', hint: 'Vergleich Russisch', smart_hint: 'Объяснение', explanation: null }, { locale: 'uk', prompt: 'Цей стіл.', hint: 'Vergleich Ukrainisch', smart_hint: null, explanation: null }]) } })
  expect(client.rpc.mock.calls[0][1].p_payload.fields.content).not.toHaveProperty('smart_hint')
  expect(client.rpc.mock.calls[0][1].p_payload.fields.content).not.toHaveProperty('translation_prompt')
  expect(client.rpc.mock.calls[0][1].p_payload.fields).not.toHaveProperty('translation_prompt')
...
test('invalid alternate answers are rejected before any database request', async () => {
  const invalid = { ...input, type: 'fill_in_blank' as const, content: { target_form: ['bestimmter Artikel'], text_before: '', text_after: ' Tisch.', correct_answer: 'Der', options: ['Der', 'Die'], accepted_answers: ['Der', ' DER '] } }
  expect(await saveGrammarExercise(invalid, id)).toEqual({ success: false, error: 'invalid' })
  expect(createClient).not.toHaveBeenCalled()
})

test.each([undefined, [], [''], ['  '], ['heißen', ''], ['heißen', 7]].map(target_form => ({ target_form })))('rejects incomplete target_form $target_form before any database request', async ({ target_form }) => {
  const invalid = { ...input, content: { ...input.content, target_form } } as unknown as typeof input
  expect(await saveGrammarExercise(invalid, id)).toEqual({ success: false, error: 'invalid' })
  expect(createClient).not.toHaveBeenCalled()
})
