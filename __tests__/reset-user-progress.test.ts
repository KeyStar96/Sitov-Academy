...
it('retries a JSON deadlock without restarting or prematurely acknowledging the reset',async()=>{
 const failure=ok({error:'retry_required',message:'Reload and retry the request.',sqlstate:'40P01'})
 const b=backend([ok(token),ok([]),failure,ok(true)])
 expect(await performLearningReset(b.client)).toEqual({success:true})
 expect(b.rpc.mock.calls.filter(call=>call[0]==='begin_learning_reset')).toHaveLength(1)
 expect(b.rpc.mock.calls.filter(call=>call[0]==='finish_learning_reset')).toHaveLength(2)
 expect(b.remove).not.toHaveBeenCalled()
 const blocked=backend([failure,failure,failure])
 expect(await performLearningReset(blocked.client)).toEqual({success:false,reason:'reset_failed'})
 expect(blocked.rpc).toHaveBeenCalledTimes(3);expect(blocked.remove).not.toHaveBeenCalled()
})
