...
const old={bucket_id:'audio_submissions',object_name:'owner-123.wav'}
...
it('deletes both approved buckets through Storage before finalizing learning rows',async()=>{
 const b=backend([ok(token),ok([old,current]),ok([]),ok(true)])
 ...
 expect(b.from.mock.calls).toEqual([['audio_submissions'],['pronunciation_audio']])
 expect(b.remove.mock.calls).toEqual([[[old.object_name]],[[current.object_name]]])
...
it('a failed Storage request does not finalize or erase the persisted retry manifest',async()=>{
 const b=backend([ok(token),ok([old,current])]);b.remove.mockResolvedValueOnce(ok([])).mockResolvedValueOnce({data:null,error:new Error('network unavailable')})
...
it('rejects unexpected RPC data before it can delete assets or shared TTS files',async()=>{
 for(const data of [[{bucket_id:'assets',object_name:'avatar.png'}],null,[{bucket_id:'pronunciation_audio',object_name:''}]]){
